...
ModelId ModelManager::loadModel(const std::string &filePath, const Matrix &transform) {
...
ModelId ModelManager::loadModel(const std::string &filePath, const Vector3 &scale,
                                const Vector3 &rotation, const Vector3 &displacement) {
	const Matrix transform = utils::math::getTransformMatrix(scale, rotation, displacement);
...
ModelId ModelManager::loadModel(const GameConfig& config, const std::string& configPath) {
...
ModelId ModelManager::createCylinder(int slices, float radius, float height) {
	return createAndAddModel("cylinder", [=]() {
		...
		const Matrix transform = utils::math::getTransformMatrix(
			...
			Vector3{90.0f, 0.0f, 0.0f} * DEG2RAD,
...
ModelId ModelManager::createPlane(float width, float length, int resX, int resZ) {
...
bool ModelManager::isValid(ModelId id) const {
